...
=item B<cryptoapicert> selector (WIN32 only)

Load the certificate and private key from the Windows Certificate System Store.
Use this option instead of B<cert> and B<key>.

B<cryptoapicert> makes it possible to use any smart card supported by Windows, but
also any certificate that includes its private key.

Selector formats:

    SUBJ:<certificate substring to match>
    THUMB:<certificate thumbprint hex value>

The thumbprint hex string can easily be retrieved (copy-paste) from the Windows
